# Español Benidorm — План 8: Курс/программа наставника (Implementation Plan)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Наставник составляет персональную программу (10–12 юнитов) под уровень и цель ученика; вкладка «Учитель» показывает путь с прогрессом, ученик идёт по нему — каждый юнит открывает урок (существующий движок) с проверкой; пройденный юнит отмечается, курс продвигается.

**Architecture:** Поверх Планов 1–7. Курс хранится в `settings.course` (jsonb) — без миграции БД, автоматически синхронизируется (План 7). Переиспуются `buildProfile`, `generateLesson`/`reviewLesson`. Новое: `generateCourse` (AI), хранилище курса в `profile.js`, переработка экрана `teacher.js` (дом = курс, + свободный урок).

**Tech Stack:** HTML/CSS/JS (ES-модули, без сборки), IndexedDB, Claude API, Node 20 `node --test`.

## Global Constraints

- Интерфейс на русском; без сборки (нативные ES-модули); без бэкенда; ключи не коммитить.
- Любой динамический текст (пользователь/модель) — в DOM только через `escapeHtml`; значения модели в class-атрибутах приводить к типу (`!!`).
- AI-структуры — JSON через `extractJson`; ошибки показываются в `#tch-status`; обработчики после `await` проверяют, что экран на месте (guard по `#tch-loading`/`#tch-status`/`#tch-results`).
- Параллельные запуски (двойной тап) блокируются флагом `busy`.
- Модель по умолчанию `claude-haiku-4-5`.
- Чистые функции — тесты `node --test`; экран/AI — ручной чек-лист. Существующие 38 тестов остаются зелёными.
- `js/app.js` НЕ менять.
- Существующие интерфейсы:
  - `js/app.js`: `registerFeature(...)`
  - `js/claude.js`: `callClaude(...)`, `extractJson` импортирован; импорт промптов: `import { WORD_ENRICH_SYSTEM, DIALOGUE_SYSTEM, GRAMMAR_SYSTEM, SPEECH_COACH_SYSTEM, DAILY_WORDS_SYSTEM, LESSON_GEN_SYSTEM, LESSON_REVIEW_SYSTEM } from './prompts.js';`; есть `generateLesson(profile,topic)`, `reviewLesson(lesson,answers)`
  - `js/profile.js`: `buildProfile()`, `saveProfileNote(note,lastTopic)`, `recordLesson(entry)`, `pickNextTopic(weak,lastTopic)`; внутри использует `getSetting`/`setSetting` (db.js)
  - `js/db.js`: `getSetting`, `setSetting`
  - `js/util.js`: `escapeHtml`
  - текущий `js/teacher.js` экран `teacher` (order 5) с `render`, `renderLesson`, `checkLesson`, `startLesson`, `exerciseHtml`, `collectAnswers` — будет переработан
- Рабочая директория: `/Users/nik/Downloads/EspanolBenidorm`, ветка `plan-8-course`.

---

## File Structure (этот план)

- `js/profile.js` — добавить `getCourse`, `saveCourse`, `markUnitDone`, `nextUnit` (чистая, **тест**)
- `js/prompts.js` — добавить `COURSE_GEN_SYSTEM`
- `js/claude.js` — добавить `generateCourse(profile, goal)`
- `js/teacher.js` — переработать: дом = программа + прогресс + запуск юнита; сохранить свободный урок
- `js/db.js` — без изменений (курс в `settings`)
- `css/styles.css` — стили списка курса
- `sw.js` — поднять версию кэша `espanol-v9`
- `test/profile.test.js` — тесты `nextUnit`

---

## Task 1: Хранилище курса (`js/profile.js`)

**Files:**
- Modify: `js/profile.js` (добавить функции в конец)
- Modify: `test/profile.test.js` (добавить тесты `nextUnit`)

**Interfaces:**
- Produces:
  - `nextUnit(units): object|null` — первый юнит со `status !== 'done'`, иначе `null` (чистая).
  - `getCourse(): Promise<object|null>` — `settings.course` или `null`.
  - `saveCourse(course): Promise<void>`.
  - `markUnitDone(unitId, score): Promise<void>` — помечает юнит `status='done'`, пишет `score`.

- [ ] **Step 1: Добавить тесты в конец `test/profile.test.js`**

```js
import { nextUnit } from '../js/profile.js';

test('nextUnit: первый невыполненный юнит', () => {
  const units = [{ id: 'a', status: 'done' }, { id: 'b', status: 'todo' }, { id: 'c', status: 'todo' }];
  assert.equal(nextUnit(units).id, 'b');
});

test('nextUnit: все выполнены → null', () => {
  assert.equal(nextUnit([{ id: 'a', status: 'done' }]), null);
});

test('nextUnit: пусто/undefined → null', () => {
  assert.equal(nextUnit([]), null);
  assert.equal(nextUnit(undefined), null);
});
```

- [ ] **Step 2: Запустить тест — убедиться, что падает**

Run: `node --test test/profile.test.js`
Expected: FAIL — `nextUnit` не экспортирован.

- [ ] **Step 3: Добавить функции в конец `js/profile.js`**

```js
export function nextUnit(units) {
  return (units || []).find((u) => u.status !== 'done') || null;
}

export async function getCourse() {
  return (await getSetting('course')) || null;
}

export async function saveCourse(course) {
  await setSetting('course', course);
}

export async function markUnitDone(unitId, score) {
  const course = await getSetting('course');
  if (!course || !course.units) return;
  const u = course.units.find((x) => x.id === unitId);
  if (u) {
    u.status = 'done';
    u.score = score;
    await setSetting('course', course);
  }
}
```

- [ ] **Step 4: Запустить тест — убедиться, что проходит**

Run: `node --test test/profile.test.js`
Expected: PASS — прежние 4 + новые 3 теста зелёные.

- [ ] **Step 5: Commit**

```bash
git add js/profile.js test/profile.test.js
git commit -m "feat: course storage helpers and nextUnit with tests"
```

---

## Task 2: Промпт и генерация курса (`js/prompts.js`, `js/claude.js`)

**Files:**
- Modify: `js/prompts.js` (добавить константу)
- Modify: `js/claude.js` (расширить импорт + добавить функцию)

**Interfaces:**
- Consumes: `callClaude`, `extractJson`, `COURSE_GEN_SYSTEM`.
- Produces: `generateCourse(profile, goal): Promise<{ units: {title, topic}[] }>`.

- [ ] **Step 1: Добавить в конец `js/prompts.js`**

```js
export const COURSE_GEN_SYSTEM = `Ты — преподаватель испанского, составляешь персональную программу обучения для русскоязычного ученика. Тебе дают профиль (уровень, выучено слов, слабые темы) и цель.
Составь последовательный курс из 10–12 коротких юнитов от простого к сложному под цель и уровень, с упором на бытовой испанский района Бенидорма (Валенсийское сообщество) и на слабые темы ученика.
Верни СТРОГО один JSON-объект, без markdown:
{
  "units": [
    { "title": "короткое название юнита по-русски", "topic": "конкретная тема для урока: грамматика или бытовая ситуация" }
  ]
}
Ровно 10–12 элементов в "units". Только валидный JSON.`;
```

- [ ] **Step 2: Расширить импорт промптов в `js/claude.js`**

Заменить строку:
```js
import { WORD_ENRICH_SYSTEM, DIALOGUE_SYSTEM, GRAMMAR_SYSTEM, SPEECH_COACH_SYSTEM, DAILY_WORDS_SYSTEM, LESSON_GEN_SYSTEM, LESSON_REVIEW_SYSTEM } from './prompts.js';
```
на:
```js
import { WORD_ENRICH_SYSTEM, DIALOGUE_SYSTEM, GRAMMAR_SYSTEM, SPEECH_COACH_SYSTEM, DAILY_WORDS_SYSTEM, LESSON_GEN_SYSTEM, LESSON_REVIEW_SYSTEM, COURSE_GEN_SYSTEM } from './prompts.js';
```

- [ ] **Step 3: Добавить функцию в конец `js/claude.js`**

```js
export async function generateCourse(profile, goal) {
  const text = await callClaude({
    system: COURSE_GEN_SYSTEM,
    messages: [{ role: 'user', content: `Профиль ученика: ${JSON.stringify(profile)}\nЦель: ${goal}` }],
    maxTokens: 1100,
  });
  return extractJson(text);
}
```

- [ ] **Step 4: Проверка импорта в Node**

Run: `node --input-type=module -e "import('./js/claude.js').then(m=>console.log(typeof m.generateCourse))"`
Expected: `function`.

- [ ] **Step 5: Commit**

```bash
git add js/prompts.js js/claude.js
git commit -m "feat: generateCourse AI syllabus generator"
```

---

## Task 3: Переработка экрана «Учитель» (`js/teacher.js`)

**Files:**
- Modify: `js/teacher.js` (полная замена содержимого)

**Interfaces:**
- Consumes: `registerFeature` (app.js), `buildProfile`/`saveProfileNote`/`recordLesson`/`getCourse`/`saveCourse`/`markUnitDone`/`nextUnit` (profile.js), `generateLesson`/`reviewLesson`/`generateCourse` (claude.js), `escapeHtml` (util.js).
- Produces: экран `teacher` (Учитель, `order: 5`) с программой и уроками.

- [ ] **Step 1: Заменить содержимое `js/teacher.js` полностью**

```js
import { registerFeature } from './app.js';
import { buildProfile, saveProfileNote, recordLesson, getCourse, saveCourse, markUnitDone, nextUnit } from './profile.js';
import { generateLesson, reviewLesson, generateCourse } from './claude.js';
import { escapeHtml } from './util.js';

const DEFAULT_GOAL = 'Разговорный бытовой испанский для повседневной жизни в районе Бенидорма';
let lesson = null;
let currentUnitId = null;
let busy = false;

function profileCard(profile, e) {
  return `<div class="study-card">
    <div class="word-ex">Уровень: <b>${e(profile.level)}</b> · выучено слов: <b>${profile.learned}</b> · уроков: <b>${profile.lessonsCompleted}</b></div>
    ${profile.note ? `<div class="word-local">📝 ${e(profile.note)}</div>` : ''}
  </div>`;
}

function exerciseHtml(ex, i) {
  const e = escapeHtml;
  if (ex.type === 'choice') {
    const opts = (ex.options || []).map((o, j) =>
      `<label class="opt-row"><input type="radio" name="ex${i}" value="${j}"> ${e(o)}</label>`).join('');
    return `<div class="lesson-ex"><div class="ex-prompt">${i + 1}. ${e(ex.prompt)}</div>${opts}</div>`;
  }
  return `<div class="lesson-ex"><div class="ex-prompt">${i + 1}. ${e(ex.prompt)}</div><textarea data-open="${i}" rows="2" placeholder="Твой ответ…"></textarea></div>`;
}

function collectAnswers(container) {
  return lesson.exercises.map((ex, i) => {
    if (ex.type === 'choice') {
      const sel = container.querySelector(`input[name="ex${i}"]:checked`);
      return sel ? ((ex.options || [])[Number(sel.value)] || '') : '';
    }
    const ta = container.querySelector(`[data-open="${i}"]`);
    return ta ? ta.value.trim() : '';
  });
}

function renderLesson(container) {
  const e = escapeHtml;
  container.innerHTML = `
    <h1>Урок: ${e(lesson.topic)}</h1>
    <div class="study-card"><div class="word-ex">${e(lesson.explanation)}</div></div>
    <div id="tch-ex">${(lesson.exercises || []).map((ex, i) => exerciseHtml(ex, i)).join('')}</div>
    <button id="tch-check">Проверить ответы</button>
    <button id="tch-cancel" class="danger">К программе</button>
    <p id="tch-status" class="status"></p>
    <div id="tch-results"></div>
  `;
  container.querySelector('#tch-check').onclick = () => checkLesson(container);
  container.querySelector('#tch-cancel').onclick = () => render(container);
}

async function checkLesson(container) {
  if (busy) return;
  busy = true;
  const status = container.querySelector('#tch-status');
  const answers = collectAnswers(container);
  if (status) status.textContent = 'Проверяю…';
  try {
    const r = await reviewLesson(lesson, answers);
    if (!container.querySelector('#tch-results')) return;
    if (status) status.textContent = '';
    const e = escapeHtml;
    const exCount = (lesson.exercises || []).length;
    const resList = (r.results || []).slice(0, exCount);
    const results = resList.map((res, i) =>
      `<div class="lesson-ex"><div class="${!!res.correct ? 'gr-ok' : 'gr-bad'}">${!!res.correct ? '✅' : '❌'} ${i + 1}</div><div class="word-ex">${e(res.comment || '')}</div></div>`).join('');
    const correct = resList.filter((x) => !!x.correct).length;
    const score = `${correct}/${exCount}`;
    container.querySelector('#tch-results').innerHTML = `
      ${results}
      <div class="study-card"><b>Итог: ${score}</b>
        <div class="word-ex">${e(r.summary || '')}</div>
        ${r.nextTopic ? `<div class="word-local">▶︎ Следующая тема: ${e(r.nextTopic)}</div>` : ''}
      </div>
      <button id="tch-back">К программе</button>
    `;
    await saveProfileNote(r.profileNote || '', lesson.topic);
    await recordLesson({ topic: lesson.topic, date: Date.now(), score });
    if (currentUnitId) await markUnitDone(currentUnitId, score);
    const back = container.querySelector('#tch-back');
    if (back) back.onclick = () => render(container);
  } catch (err) {
    const s = container.querySelector('#tch-status');
    if (s) s.textContent = err.message;
  } finally {
    busy = false;
  }
}

async function startLesson(container, topic, unitId) {
  if (busy) return;
  busy = true;
  currentUnitId = unitId || null;
  const status = container.querySelector('#tch-status');
  if (status) status.textContent = 'Готовлю урок под твой уровень…';
  try {
    const profile = await buildProfile();
    lesson = await generateLesson(profile, topic);
    if (!container.querySelector('#tch-status')) return;
    renderLesson(container);
  } catch (err) {
    const s = container.querySelector('#tch-status');
    if (s) s.textContent = err.message;
  } finally {
    busy = false;
  }
}

async function buildCourse(container) {
  if (busy) return;
  busy = true;
  const goalEl = container.querySelector('#tch-goal');
  const goal = (goalEl && goalEl.value.trim()) || DEFAULT_GOAL;
  const status = container.querySelector('#tch-status');
  if (status) status.textContent = 'Составляю программу под тебя…';
  try {
    const profile = await buildProfile();
    const gen = await generateCourse(profile, goal);
    const units = (gen.units || []).map((u, i) => ({
      id: `u${Date.now()}_${i}`, title: u.title || '', topic: u.topic || '', status: 'todo', score: '',
    }));
    await saveCourse({ goal, units, createdAt: Date.now() });
    if (!container.querySelector('#tch-status')) return;
    render(container);
  } catch (err) {
    const s = container.querySelector('#tch-status');
    if (s) s.textContent = err.message;
  } finally {
    busy = false;
  }
}

async function render(container) {
  container.innerHTML = '<h1>Учитель</h1><p class="status" id="tch-loading">Анализирую твой уровень…</p>';
  const profile = await buildProfile();
  if (!container.querySelector('#tch-loading')) return;
  const course = await getCourse();
  const e = escapeHtml;

  if (course && course.units && course.units.length) {
    const done = course.units.filter((u) => u.status === 'done').length;
    const total = course.units.length;
    const next = nextUnit(course.units);
    const unitsHtml = course.units.map((u, i) => {
      const isNext = next && u.id === next.id;
      const mark = u.status === 'done' ? '✅' : (isNext ? '▶︎' : '•');
      return `<div class="unit-row${isNext ? ' unit-next' : ''}">
        <span class="unit-mark">${mark}</span>
        <span class="unit-title">${i + 1}. ${e(u.title)}</span>
        ${u.score ? `<span class="unit-score">${e(u.score)}</span>` : ''}
      </div>`;
    }).join('');
    container.innerHTML = `<h1>Учитель</h1>${profileCard(profile, e)}
      <div class="study-card">
        <div class="word-ex">Программа: <b>${e(course.goal)}</b></div>
        <div class="daily-progress">Пройдено ${done}/${total}</div>
      </div>
      <div class="course-list">${unitsHtml}</div>
      ${next ? `<button id="tch-start">▶︎ Начать: ${e(next.title)}</button>`
             : '<p class="status">🎉 Курс пройден! Можно пересоставить программу или взять свободную тему.</p>'}
      <details class="tch-extra"><summary>Другое</summary>
        <label>Свободная тема урока<input id="tch-topic" type="text" placeholder="напр. прошедшее время"></label>
        <button id="tch-free">Свободный урок</button>
        <button id="tch-rebuild" class="danger">Пересоставить программу</button>
      </details>
      <p id="tch-status" class="status"></p>`;
    if (next) {
      container.querySelector('#tch-start').onclick = () => startLesson(container, next.topic, next.id);
    }
    container.querySelector('#tch-free').onclick = () => {
      const t = container.querySelector('#tch-topic').value.trim();
      if (t) startLesson(container, t, null);
    };
    container.querySelector('#tch-rebuild').onclick = () => buildCourse(container);
  } else {
    container.innerHTML = `<h1>Учитель</h1>${profileCard(profile, e)}
      <div class="study-card"><div class="word-ex">У тебя ещё нет программы. Составлю персональный курс под твой уровень и цель.</div></div>
      <label>Цель обучения<input id="tch-goal" type="text" value="${e(DEFAULT_GOAL)}"></label>
      <button id="tch-build">📚 Составить программу</button>
      <details class="tch-extra"><summary>Или сразу свободный урок</summary>
        <label>Тема<input id="tch-topic" type="text" placeholder="напр. артикли"></label>
        <button id="tch-free">Свободный урок</button>
      </details>
      <p id="tch-status" class="status"></p>`;
    container.querySelector('#tch-build').onclick = () => buildCourse(container);
    container.querySelector('#tch-free').onclick = () => {
      const t = container.querySelector('#tch-topic').value.trim();
      if (t) startLesson(container, t, null);
    };
  }
}

registerFeature({ id: 'teacher', title: 'Учитель', icon: '👨‍🏫', order: 5, render });
```

- [ ] **Step 2: Commit**

```bash
git add js/teacher.js
git commit -m "feat: teacher home as personal course with progress and unit lessons"
```

---

## Task 4: Стили курса, кэш, проверка

**Files:**
- Modify: `css/styles.css`, `sw.js`

- [ ] **Step 1: Добавить стили в конец `css/styles.css`**

```css
.course-list { margin: 12px 0; }
.unit-row { display: flex; align-items: center; gap: 10px; padding: 11px 12px; margin: 6px 0;
  background: var(--card); border: 1px solid var(--border); border-radius: 12px; }
.unit-row.unit-next { border-color: var(--accent); box-shadow: 0 0 0 2px rgba(124,108,255,.25); }
.unit-mark { width: 20px; text-align: center; }
.unit-title { flex: 1; font-size: 15px; }
.unit-score { color: var(--muted); font-size: 13px; }
.tch-extra { margin-top: 14px; }
.tch-extra summary { color: var(--muted); cursor: pointer; padding: 8px 0; }
```

- [ ] **Step 2: Обновить кэш в `sw.js`**

Заменить `const CACHE = 'espanol-v8';` на `const CACHE = 'espanol-v9';`.
(Новых файлов нет — `teacher.js`, `profile.js`, `claude.js`, `prompts.js` уже в `SHELL`; версия поднимается, чтобы клиенты подтянули обновления.)

- [ ] **Step 3: Прогнать все тесты**

Run: `npm test`
Expected: PASS — все наборы зелёные (38 прежних + 3 новых `nextUnit` = 41), 0 провалов.

- [ ] **Step 4: Ручная проверка в Chrome**

Run: `npm start` (http://localhost:3000), вкладка 👨‍🏫 «Учитель»:
- Первый заход (курса нет): карточка профиля + поле «Цель» (предзаполнено) + «📚 Составить программу». Нажать → через пару секунд появляется список из 10–12 юнитов, первый помечен ▶︎, «Пройдено 0/N».
- «▶︎ Начать: …» → генерится урок (объяснение + упражнения) → «Проверить ответы» → разбор + итог → «К программе».
- После возврата: пройденный юнит помечен ✅ со счётом, следующий стал ▶︎, «Пройдено 1/N».
- «Другое» → «Свободный урок» по своей теме работает как раньше (юнит не помечается).
- DevTools → IndexedDB → `settings` → ключ `course` хранит `{goal, units[], createdAt}`.
- Перезагрузка страницы: курс и прогресс на месте.

- [ ] **Step 5: Commit**

```bash
git add css/styles.css sw.js
git commit -m "feat: course list styles, cache v9"
```

---

## Self-Review

**Spec coverage (План 8 = «Курс/программа» из плана наставника):**
- Персональная программа под уровень и цель → Tasks 1, 2, 3 (`generateCourse` + сохранение). ✔
- Путь с прогрессом «Пройдено N/M», текущий юнит → Task 3 (`nextUnit`, список). ✔
- Каждый юнит → урок (существующий движок) с проверкой → Task 3 (`startLesson`/`checkLesson`). ✔
- Пройденный юнит отмечается, курс продвигается → Tasks 1, 3 (`markUnitDone`). ✔
- Свободный урок по теме сохранён → Task 3. ✔
- Курс в `settings` (синхронизируется, без миграции) → Task 1. ✔
- Безопасная вставка (escapeHtml), `!!correct`, guard, `busy` → Task 3. ✔
- `js/app.js` не тронут; офлайн-кэш поднят → Task 4. ✔
- (Чат, Задания, План на сегодня — Планы 9–11.)

**Placeholder scan:** Полный код во всех шагах. Ошибки AI ловятся в `#tch-status`. Нет TODO без кода.

**Type consistency:** `nextUnit`/`getCourse`/`saveCourse`/`markUnitDone` (Task 1) → Task 3; `COURSE_GEN_SYSTEM` (Task 2) → claude импорт Task 2; `generateCourse` (Task 2) → Task 3; форма курса (`{goal, units:[{id,title,topic,status,score}], createdAt}`) согласована между `buildCourse`, `render`, `markUnitDone`; урок (`{topic,explanation,exercises}`) и ревью (`{results,summary,nextTopic,profileNote}`) — как в Плане 6. ✔
```
